...
builder.Services.AddHttpClient<IWebsiteScraperService, MyMediaVerse.Infrastructure.Services.WebsiteScraperService>(client =>
{
...
builder.Services.AddHttpClient<IWebsiteScreenshotService, MyMediaVerse.Infrastructure.Services.WebsiteScreenshotService>(client =>
{
    client.DefaultRequestHeaders.Add("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36");
    client.Timeout = TimeSpan.FromSeconds(30);
});
...
builder.Services.AddHttpClient<IRssFeedService, MyMediaVerse.Infrastructure.Services.RssFeedService>(client =>
{
    client.DefaultRequestHeaders.Add("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36");
                ...
                try
                {
                    ...
                    if (separatorIndex > 0)
                    {
                        ...
                        var expectedUsername = Environment.GetEnvironmentVariable("AUTH_USERNAME")
                            ?? builder.Configuration["Auth:Username"];
